...
def passive_tests(url, headers):
    """Run passive checks against a response for CORS issues.
    ...
    -------
    dict or None
        A dictionary describing the detected misconfiguration, if any.
    """
    root = host(url)
...
def active_tests(
    ...
    """Perform active CORS checks by sending requests with crafted origins.
    ...
    url : str
        Target URL to test.
    root : str
        Base domain extracted from ``url``.
    scheme : str
        HTTP scheme (``http`` or ``https``) used for the requests.
    header_dict : dict
        Headers to include with each request.
    delay : int or float
        Seconds to wait between individual tests.
    timeout : int, optional
        Socket timeout for each request, by default ``10``.
    verify : bool, optional
        Whether to verify TLS certificates, by default ``True``.

    Returns
    -------
    dict or None
        Details of a discovered misconfiguration or ``None`` if no
        issue is found.
    """
    origin = scheme + "://" + root
    headers = requester(
        url, scheme, header_dict, origin, timeout=timeout, verify=verify
    )
    acao_header, acac_header = headers.get(
        "access-control-allow-origin", None
    ), headers.get("access-control-allow-credentials", None)
    ...
    origin = "http://" + root
    headers = requester(
        url, "http", header_dict, origin, timeout=timeout, verify=verify
    )
    acao_header, acac_header = headers.get(
        "access-control-allow-origin", None
    ), headers.get("access-control-allow-credentials", None)
